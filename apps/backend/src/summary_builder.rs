...
impl SummaryBuilder {
    ...
    pub async fn build_overview(&self) -> Result<Overview, String> {
        // Fetch from Redis
        let agents = self.redis_reader.get_agents_status().await.unwrap_or_default();
        let queue_summary = self.redis_reader.get_queue_summary().await.unwrap_or(QueueSummary {
            pending_critical: 0,
            pending_high: 0,
            pending_normal: 0,
            pending_low: 0,
    ...
    pub async fn build_usage_summary(&self) -> Result<UsageSummary, String> {
        ...
        // Aggregate by agent
        let mut by_agent_map: std::collections::HashMap<String, (i32, f64)> = std::collections::HashMap::new();
        for usage in &usages {
        ...
        // Aggregate by model
        let mut by_model_map: std::collections::HashMap<String, (i32, f64)> = std::collections::HashMap::new();
        for usage in &usages {
        ...
        let mut by_day_map: std::collections::HashMap<String, (i32, f64)> = std::collections::HashMap::new();
        for usage in &usages {
